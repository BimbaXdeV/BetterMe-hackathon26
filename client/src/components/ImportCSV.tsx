...
interface Props {
  // Обновляем интерфейс, чтобы он мог принимать объект статистики
  onSuccess?: (stats?: { count: number; tax: number; total: number }) => void; 
}
      ...
      complete: async (results) => {
        try {
          ...
          for (let i = 0; i < total; i += chunkSize) {
            ...
            // Сохраняем ответ последнего запроса
            const response = await axios.post('http://localhost:8000/orders/import', chunk);
          ...
          // Если есть данные в ответе, берем статистику из последнего объекта массива
          if (onSuccess && lastResponseData && lastResponseData.length > 0) {
            const lastItem = lastResponseData[lastResponseData.length - 1];
            onSuccess({
              count: lastItem.orders_count,
              tax: lastItem.total_tax,
              total: lastItem.total_revenue
            });
          } else if (onSuccess) {
            onSuccess();
          }
